...
- 세션 시간은 기존 `SessionTimer`의 7개 duration 값을 그대로 기록하고 별도 계산 루프를 만들지 않는다.
- IndexedDB `studylog-db`에는 세션 요약, 1초 표본, 모델 평가 결과만 저장한다.
- 원본 영상·이미지·MediaPipe landmark 배열은 IndexedDB와 CSV에 저장하지 않는다.
